...
webctl 0.1.5 (behavior version 0.0.028). 30 research questions, three agent harnesses, one run per cell, 2026-09-20. Judge: Kimi K3 via pi, blind to arm, all arms of a case graded in one call.

Durable evidence lives in `experiments/<name>/results/`: `cells.jsonl` (one line per case × arm: answer, tokens, cost, payload, tool calls, verdict), `meta.json`, and `report.md`. The tables below come from `experiments/2026-09-20-03-no-scrape-lite`; `webctl-bench report 2026-09-20-03-no-scrape-lite` regenerates the full report. Experiments 01 and 02 are earlier states of the same cases and are kept for the record.

Arms: each harness was told to use only its own web search (native), only `webctl search` (no scrape), or only webctl with `--scrape --filter-chunks` (scrape). pi has no built-in search.

## Headline

Search payload is what the search tool returned into the agent's context, in tokens (chars ÷ 4). Total tokens include the harness's own per-turn baseline, about 20k on Claude and 16k on Codex, which no search tool can change.
...
- **Claude Code:** webctl beats the built-in search by about a point of quality either way, cites a source every time, and costs 19 to 30% less. Without scraping the payload is within 50% of native links; with scraping it is 4.5×. Total tokens drop 17 to 28% because webctl needs fewer turns.
- **Codex:** quality is a wash. webctl cuts total tokens 25 to 32% but is 22 to 26% slower: a shell round-trip loses to server-side search. Codex's native payload is server-side and not observable.
...
Scraping triples the payload for 0.07 points on Claude and 0.17 on Codex. Quality by domain, native / no scrape / scrape, payload in parentheses:
...
Scrape pays on earnings transcripts, community threads, and long documents, where the answer is a paragraph deep inside a page. It adds nothing on dev docs, news, or short facts. Hence the help text: work from snippets, scrape when you would otherwise read the whole page.
...
- webctl's own Jev calls are billed to the Jev key and are in no token count.
